...
logging = configure_logger("data_validation")
...
class DataValidation:
    ...
    def is_column_exist(self,dataframe:pd.DataFrame):
        ...
        if len(missing_numerical_column)> 0:
            logging.info(f"Missing numerical column {missing_numerical_column}")
        ...
        if len(missing_categorical_column)>0:
            logging.info(f"Missing categorical column {missing_categorical_column}")
